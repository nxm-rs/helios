...
// alloy 2.0 reshaped the builder traits: getters and setters live on
// the base `TransactionBuilder` (which alloy itself provides for
// `TransactionRequest`), and build / submit / type-selection methods
// moved to `NetworkTransactionBuilder<N>`. The local `Ethereum`
// network type means we still need our own `NetworkTransactionBuilder`
// impl, but the bodies are identical to alloy's `Ethereum` impl, so we
// delegate where there's a one-liner equivalent on `TransactionRequest`
// and inline the rest.
